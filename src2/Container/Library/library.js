...
import {LOGIN,TAG,TOKEN,USERID,HEADER,CHANGE_STATUS,CREATECATEGORY,INSERTLIBRARY,GETLIBRARY,CATEGORYDATABYID,toUpperCaseFilter} from '../../url.js';
...
class Library extends React.Component{
  ...
  getData=(page,keyword)=>{
    axios.post(`${GETLIBRARY}?page=${page}`,{
      'tag':'dash',
  ...
  // });
  // }
  createSingleLibrary=(e)=>{
    e.preventDefault();
    var name=e.target.name.value.trim();
    var short_name=e.target.short_name.value.trim();
    var duration=e.target.duration.value.trim();
    var channel_id=e.target.channel_id.value.trim();
    var category_id=e.target.category_id.value.trim();
    var rights=e.target.rights.value.trim();
    var content_id=e.target.content_id.value.trim();
    var res={"name":name,"short_name":short_name,"duration":duration,"channel_id":channel_id,
  "category_id":category_id,"rights":rights,"content_id":content_id,"status":1};
  var data=[];
  data.push(res);
    axios.post(INSERTLIBRARY,
      {
        user_id:USERID,
        data:data,
        tag:TAG
  ...
  render()
  {
    ...
    const tableContent=(categoryList.length >0?categoryList.map((res,key)=>{
      return(<tr key={key}><td className="tbl_l">{res.content_id}</td>
                <td>{res.name}</td>
                <td>{res.add_categories!=null?res.add_categories.name:"Null"}</td>
    ...
    if(isLoading)
    {
      return(<div class="cont_area">
          ...
          <div class="box_t">
            <div class="buttons">
                <div class="dropdown">
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="Javascript:" onClick={()=>{this.setState({modal_type:"add_single_library",open_portal:true});$("#portal_modal").addClass("show");$("body").addClass("bfix")}}><i class="fas fa-plus"></i> Add Single Content</a>
                    <a class="dropdown-item" href="Javascript:" onClick={()=>{this.setState({modal_type:"add_csv_library",open_portal:true});$("#portal_modal").addClass("show");$("body").addClass("bfix")}}><i class="fas fa-upload"></i> Upload CSV</a>
                  </div>
                </div>
            </div>
            <SearchComponent getData={this.getData} currentPage={this.state.currentPage} placeholder="Search Content"/>
          </div>
        <div>
        ...
        {(open_portal) &&<Portal modalType={modal_type} portalData={this.state.portalData} modal_loading={modal_loading} closeModal={()=>this.setState({auth_err:'',open_portal:false,modal_loading_msg:'',modal_loading:false})} loading_msg={modal_loading_msg} createSingleLibrary={this.createSingleLibrary} createCsvLibrary={this.createCsvLibrary}/>}
        {/**/}
      </div>)
    }
    else
